...
public class Server {
    public static void main(String[] args) throws IOException {
        ...
        serverChannel.bind(new InetSocketAddress("localhost", 25999));
        while (true) {
            ...
            try (SocketChannel socketChannel = serverChannel.accept()) {
                ...
                while (socketChannel.isConnected()) {
                    ...
                    inputBuffer.clear();
                    System.out.println("Получена строка для обработки: " + msg);
                    //  отправляем сообщение клиента назад с пометкой ЭХО
                    socketChannel.write(ByteBuffer.wrap(("Строка без пробелов: " + msg).getBytes(StandardCharsets.UTF_8)));
                }
